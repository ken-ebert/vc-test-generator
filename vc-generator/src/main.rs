...
*/
	...
	// Is the test file a zkp file?
	let zkp_re = Regex::new(r".*(?P<zkp_str>zkp).*\.jsonld").unwrap(); 
	//let zkp_caps = zkp_re.captures(&filename).unwrap();
	let mut zkp_format = false;
	let zkp_caps = match zkp_re.captures(&filename){
		Some(Captures) => {
			//println!("ZKP proof format");
			zkp_format = true;
		},
		None => {
			//println!("Not a ZKP proof format");
		}
	};
	
	// FIXME how do we look at the captures and not die on unwrap?
	//println!("zkp_caps = {}", zkp_caps);
	...
	if schema_json.credential_schema != Null {
		...
		match schema_json.credential_schema {
    		...
    		//Array(a) => println!("    {},", schema_json.credential_schema.to_string()),
    		Array(a) => {
 				println!("[");
				let mut need_schema_comma = false;
    			for schema_value in a {
    				if need_schema_comma {
    					// only print commas in front of 2nd to nth object
    					println!(",");
    				}
    				// output each id & type
    				println!("    {{");
					match schema_value {
						Null => panic!("credentialSchema array is Null."),
						Bool(_ab) => panic!("credentialSchema array is Bool \"{}\"", _ab),
						Number(_an) => panic!("credentialSchema array is Number \"{}\"", _an),
    					serde_json::value::Value::String(_as) => panic!("credentialSchema array is string \"{}\"", _as),
			    		Array(_aa) => panic!("credentialSchema array is another array"),
						Object(_am) => {
							if _am["id"] != Null {
								println!("      \"id\": {},", _am["id"]);
							} else {
					...
					need_schema_comma = true;
    			}
    			// End array of schemas
				print!("  ],"); 
    		}
    		Object(m) => {
				println!("{{");
		...
		if zkp_format {
			panic!("ZKP verifiable credential missing credentialSchema");
		}
	}
